...
def add_special_enemies(group1, group2, num):
    for i in range(num):
        e4 = enemy.SpecialEnemy(bg_size) #创建横飞的特殊小飞机实例化对象
        group1.add(e4)
        group2.add(e4)
...
def main():
    ...
    for i in range(BULLET3_NUM // 5):
        ...
        bullet3.append(bullet.Bullet1((me.rect.centerx-27, me.rect.centery-21)))
        bullet3.append(bullet.Bullet1((me.rect.centerx+24, me.rect.centery-21)))
    ...
    while running:
        for event in pygame.event.get(): #检测事件循环
            ...
            #触发特殊飞机事件
                ...
                #生成特殊飞机
                add_special_enemies(special_enemies, enemies, special_enemies_num)#调用方法，用来把飞机添加到Group中
                    ...
                    if bullet2_level < 8:
                        ...
                        if bullet2_level == 1: # 1级：每次发2颗
                            ...
                            for i in range(BULLET2_NUM // 4):
                                bullet2.append(bullet.Bullet2((me.rect.centerx-43, me.rect.centery)))
                                bullet2.append(bullet.Bullet2((me.rect.centerx+40, me.rect.centery)))
                            ...
                            for i in range(BULLET2_NUM // 5):
                                bullet2.append(bullet.Bullet2((me.rect.centerx-58, me.rect.centery)))
                                bullet2.append(bullet.Bullet2((me.rect.centerx+55, me.rect.centery)))
                                bullet2.append(bullet.Bullet2((me.rect.centerx-27, me.rect.centery-21)))
                                bullet2.append(bullet.Bullet2((me.rect.centerx+24, me.rect.centery-21)))
                                bullet2.append(bullet.Bullet2(me.rect.midtop))
                ...
                for each in special_enemies:
                    if each.active and not each.death: #要还在活动的和之前还没死过的小飞机才能move
                        ...
                        if not(delay % 3): #每次当delay能整除3的时候，就显示一张图片
                            ...
                            e4_destroy_index = (e4_destroy_index + 1) % 1 #这里是一个小技巧，这样 e1_destroy_index的值只能是0~3
                            if e4_destroy_index == 0: #当飞机毁灭图片显示完时，就把小飞机移除出group
                                each.reset() #重置飞机位置，并且active = True, death = True
            ...
            if not(delay % 10): #每10帧重置一次图片
                ...
                if is_double_bullet and bullet2_level > 0: #如果是超级子弹
                    if bullet2_level == 1:
                        ...
                        bullet2_index = (bullet2_index + 3) % BULLET2_NUM #索引+1
                    elif bullet2_level == 3:
                        bullets = bullet2
                        bullets[bullet2_index].reset((me.rect.centerx-43, me.rect.centery))
                        bullets[bullet2_index+1].reset((me.rect.centerx+40, me.rect.centery))
                        bullets[bullet2_index+2].reset((me.rect.centerx-18, me.rect.centery))
                        bullets[bullet2_index+3].reset((me.rect.centerx+15, me.rect.centery))
                        bullet2_index = (bullet2_index + 4) % BULLET2_NUM #索引+1
                    elif 4 <= bullet2_level < 8:
                        bullets = bullet2
                        bullets[bullet2_index].reset((me.rect.centerx-58, me.rect.centery))
                        bullets[bullet2_index+1].reset((me.rect.centerx+55, me.rect.centery))
                        bullets[bullet2_index+2].reset((me.rect.centerx-27, me.rect.centery-21))
                        bullets[bullet2_index+3].reset((me.rect.centerx+24, me.rect.centery-21))
                        bullets[bullet2_index+4].reset(me.rect.midtop)
                        bullet2_index = (bullet2_index + 5) % BULLET2_NUM #索引+1

                elif bullet2_level >= 8:
                    bullets = bullet3
                    bullets[bullet3_index].reset((me.rect.centerx-58, me.rect.centery))
                    bullets[bullet3_index+1].reset((me.rect.centerx+55, me.rect.centery))
                    bullets[bullet3_index+2].reset((me.rect.centerx-27, me.rect.centery-21))
                    bullets[bullet3_index+3].reset((me.rect.centerx+24, me.rect.centery-21))
